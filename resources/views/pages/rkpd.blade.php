                ...
                <div class="table-responsive mt-4">
                  <table class="table table-hover border text-center text-nowrap" id="dataTable1">
                    ...
                    <tbody>
                      ...
                      @php
                        if ($item->rkpd->count() <= 2) {
                          $color = 'danger';
                        } elseif ($item->rkpd->count() <=5) {
                          $color = 'warning';
                        } else {
                          $color = 'success';
                        }
                        $percent = number_format($item->rkpd->count() / 7 * 100, 0);
                      @endphp
                      <tr>
                        <td>{{ $item->name }}</td>
                        <td class="button-for-upload">
                          @if ($item->rkpd->count() != 7)
                            @for ($i = 1; $i <= 7; $i++)
                            <button class="btn btn-sm btn-{{ $item->rkpd->count() >= $i ? $color : 'outline-secondary' }}" data-bs-toggle="modal" data-bs-target="#uploadModal-{{ $item->id }}">
                              {{ $i }}
                            </button>
                            @endfor
                          @else
                        ...
                        <td>
                          @if ($item->rkpd->count())
                            {{ Carbon\Carbon::parse($item->latestRkpd->created_at)->isoFormat('D MMM YYYY') }}
                          @else
                            -
                          @endif
                        </td>
...
@include('includes.modal.rkpd-modal')
@endsection
